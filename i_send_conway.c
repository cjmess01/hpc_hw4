...
int conway(short int **map1, short int **map2, int nCols, int nRows, int comm_sz, int local_start, int local_end, int my_rank, int partition){
  ...
  if(comm_sz > 1){
    MPI_Request send1, recv1;

    // Send my map's value up the chain (IE, Thread 1 sends up to 2)
    if(my_rank != comm_sz -1 && my_rank != 0){
      MPI_Isend(&(map1[local_start][0]), partition, MPI_SHORT, (my_rank+1), 0, MPI_COMM_WORLD, &send1);
      MPI_Wait(&send1, MPI_STATUS_IGNORE);

      MPI_Irecv(&map1[((my_rank-1)*partition / nCols + 1)][0], partition, MPI_SHORT, (my_rank-1), 0, MPI_COMM_WORLD, &recv1);
      MPI_Wait(&recv1, MPI_STATUS_IGNORE);
    } else{
      // If the first/last one, they should only send/recv, respectively
      if(my_rank == comm_sz-1){
        MPI_Irecv(&map1[((my_rank-1)*partition / nCols + 1)][0], partition, MPI_SHORT, comm_sz-2, 0, MPI_COMM_WORLD, &recv1);
        MPI_Wait(&recv1, MPI_STATUS_IGNORE);
      }else {
        MPI_Isend(&(map1[local_start][0]), partition, MPI_SHORT, (my_rank+1), 0, MPI_COMM_WORLD, &send1);  
        MPI_Wait(&send1, MPI_STATUS_IGNORE);
    ...
    if(my_rank != comm_sz -1 && my_rank != 0){
      ...
      MPI_Isend(&(map1[local_start][0]), partition, MPI_SHORT, (my_rank-1), 0, MPI_COMM_WORLD, &send2);
      MPI_Wait(&send2, MPI_STATUS_IGNORE);

      MPI_Irecv(&map1[((my_rank+1)*partition / nCols + 1)][0], partition, MPI_SHORT, (my_rank+1), 0, MPI_COMM_WORLD, &recv2);
      MPI_Wait(&recv2, MPI_STATUS_IGNORE);
    
    } else{
      if(my_rank == comm_sz-1){
        MPI_Isend(&(map1[local_start][0]), partition, MPI_SHORT, comm_sz-2, 0, MPI_COMM_WORLD, &send2);
        MPI_Wait(&send2, MPI_STATUS_IGNORE);
      }

      else {
        MPI_Irecv(&map1[((my_rank+1)*partition / nCols + 1)][0], partition, MPI_SHORT, 1, 0, MPI_COMM_WORLD, &recv2);  
        MPI_Wait(&recv2, MPI_STATUS_IGNORE);
      }// Process 0
...
int main(int argc, char **argv)
{
  ...
  if (comm_sz == 16)
  {
    ...
    if (my_rank == 0)
    {
      ...
      MPI_Isend(&(map1[4681][0]), special_partition_size, MPI_SHORT, 15, 0, MPI_COMM_WORLD, &send1);
      MPI_Wait(&send1, MPI_STATUS_IGNORE);
    }
    else if (my_rank == 15)
    {
      // Receive extra rows
      MPI_Irecv(&map2[local_start][0], special_partition_size, MPI_SHORT, 0, 0, MPI_COMM_WORLD, &recv1);
      MPI_Wait(&recv1, MPI_STATUS_IGNORE);
    }
  }
  MPI_Barrier(MPI_COMM_WORLD);
  ...
  if (my_rank == 0 && comm_sz > 1)
  {
    for (int i = 1; i < comm_sz; i++)
    {
      ...
      if (comm_sz == 16)
      {
        if (i == comm_sz - 1)
        {
          MPI_Irecv(&(map1[4681][0]), special_partition_size, MPI_SHORT, i, 0, MPI_COMM_WORLD, &recv1);
          MPI_Wait(&recv1, MPI_STATUS_IGNORE);
        }
        else
        {
          MPI_Irecv(&(map1[i * 312 + 1][0]), partition_size, MPI_SHORT, i, 0, MPI_COMM_WORLD, &recv1);
          MPI_Wait(&recv1, MPI_STATUS_IGNORE);
        }
      ...
      {
        MPI_Irecv(&(map1[i * partition_size / nCols + 1][0]), partition_size, MPI_SHORT, i, 0, MPI_COMM_WORLD, &recv1);
        MPI_Wait(&recv1, MPI_STATUS_IGNORE);
      }
  ...
  else if (comm_sz > 1 && my_rank != 0)
  {
    ...
    if (comm_sz == 16)
    {
      if (my_rank != comm_sz - 1)
      {
        MPI_Isend(&(map1[local_start][0]), partition_size, MPI_SHORT, 0, 0, MPI_COMM_WORLD, &send1);
        MPI_Wait(&send1, MPI_STATUS_IGNORE);
      }
      else if (my_rank == comm_sz - 1)
      {
        MPI_Isend(&(map1[local_start][0]), special_partition_size, MPI_SHORT, 0, 0, MPI_COMM_WORLD,&send1);
        MPI_Wait(&send1, MPI_STATUS_IGNORE);
      }
    }
    else
